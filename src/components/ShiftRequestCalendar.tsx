...
import type { CalendarDate, ShiftType } from '../types';

export function ShiftRequestCalendar() {
  const [currentYear, setCurrentYear] = useState(new Date().getFullYear());
  const [currentMonth, setCurrentMonth] = useState(new Date().getMonth() + 1);
  const [selectedStaffId, setSelectedStaffId] = useState<string>('');
  const [selectedShiftType, setSelectedShiftType] = useState<ShiftType>('日勤');
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            >
              {patterns
                .filter(p => p.isWorkday || p.name === '休み')
                .map((pattern) => (
                  <option key={pattern.id} value={pattern.name}>
                    {pattern.name} ({pattern.shortName})
                  </option>
                ))}
            </select>
          </div>
